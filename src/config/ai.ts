...
2. **回答判定 (Response - 仅在 valid 时)**:
    - **yes**: 提问内容符合汤底事实。
    - **no**: 提问内容违背汤底事实。
    - **unrelated**: 提问内容在汤底中未提及，对推理故事真相无关的情况。提问内容既有可能为真，也有可能为假。
...
# Input
你会得到这些上下文信息：【汤底 (truth)】、【汤面 (surface)】【玩家的提问历史 (tries)】、【玩家的解答 (solution)】、【获得的提示次数 (hints count)】。
...
2. **评分 (score)**：
    ...
    - 评分标准：
      ...
      * 无关提问：过多的无关问题应该减分
      * **提示使用**：每使用一次提示减 5-10 分（根据提示的关键程度酌情减分）
    - 严格遵循以下标准：
      * 90-100分：3-5个关键提问，直击核心，推理路径极为清晰，未使用提示或仅使用1次
      * 80-89分：6-8个提问，逻辑合理，较快找到真相，使用提示不超过2次
      * 70-79分：9-12个提问，有一定的绕路，但最终推理正确，使用提示2-3次
      * 60-69分：13-15个提问，推理过程较为曲折，使用提示3-4次
      * 50-59分：16-20个提问，有较多无关问题，使用提示4次以上
      * 40-49分：20+个提问，推理效率较低，过度依赖提示
      * 0-39分：极多无关提问或推理方向偏离严重，过度依赖提示
...
【获得的提示次数 (Hints Count)】：${HINTS_VARIABLE}
`;
